...
function validateUnitLabelCoverage(
  ...
  const preferredVisible = Math.max(1, Math.ceil(meaningfulUnits.length * 0.65));
  if (meaningfulUnits.length >= 6 && visibleCount < preferredVisible) {
    warnings.push(`Provider marked many non-empty units as plain (${visibleCount}/${meaningfulUnits.length} visible; preferred at least ${preferredVisible}). Kept provider judgment.`);
  }
  return { warnings };
}
