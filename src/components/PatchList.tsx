...
export function PatchList({
  ...
  return (
    <section data-testid="patch-list" className="mt-2 max-h-36 overflow-auto">
        ...
        {patches.map((patch, index) => (
          <article key={patch.id} data-testid="patch-list-item" className="rounded-lg border border-amber-100 bg-amber-50/70 p-2 text-xs text-amber-950">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="font-semibold">Note {index + 1}</div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button className="rounded-md border border-amber-200 bg-white px-2 py-1 text-amber-800" onClick={() => onJump(patch)}>Jump</button>
