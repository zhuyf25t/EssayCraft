...
export function ModuleSidebar({
  ...
  return (
    <aside data-testid="module-sidebar" className="flex h-full min-h-0 w-52 shrink-0 flex-col overflow-hidden border-r border-slate-200 bg-[#fffdf7]/95 p-1.5">
      <div className="mb-1 flex shrink-0 items-start gap-2">
        ...
        {MODULES.map((moduleNumber) => {
          ...
          return (
            <button
              ...
              className={`relative z-10 grid w-full grid-cols-[1.55rem_1fr] items-center gap-1 rounded-lg border px-1.5 py-0.5 text-left transition ${
                active
            ...
            >
              <span className={`flex h-5 w-5 items-center justify-center rounded-full border-2 bg-white text-[11px] font-bold ${active ? "border-blue-600 text-blue-700" : tone === "emerald" ? "border-emerald-600 text-emerald-700" : tone === "red" ? "border-rose-300 text-rose-600" : "border-slate-300 text-slate-500"}`}>
              ...
              <span className="min-w-0">
                ...
                <span className={`mt-px inline-block rounded-full px-1.5 py-px text-[9px] leading-none ${statusClass(status)}`}>{status}</span>
              </span>
            </button>
          );
        })}
