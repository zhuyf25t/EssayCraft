...
describe("refresh validation", () => {
  ...
  it("marks explicit citation-needed placeholders as issue even when provider calls them evidence", () => {
    ...
    expect(result.warnings.join(" ")).toMatch(/evidence\/citation-needed placeholder as issue/i);
  });

  it("marks explicit evidence-needed placeholders as issue if the AI misses the problem", () => {
    const text = "This claim would be stronger with a peer-reviewed source [evidence needed].";
    const result = validateProviderRefreshAnnotations(text, [exactAnnotation(text, "analysis")], 4);

    expect(result.usedFallback).toBe(false);
    expect(result.annotations[0].label).toBe("issue");
    expect(result.annotations[0].text).toContain("[evidence needed]");
  });
