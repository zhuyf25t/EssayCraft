...
import { buildMockAnnotations, findIssueRanges, guardAgainstCitationOverlabeling } from "@/lib/annotations";
import { applyNotesFallback } from "@/lib/rewriteFallback";

export function mockRefresh(input: RefreshRequest, message: string): RefreshResponse {
...
export function mockPatchRevision(text: string, patches: Patch[], projectTitle = ""): RefreshResponse {
...
export function addModuleReviewIfNeeded(input: RefreshRequest, response: RefreshResponse): RefreshResponse {
...
export function moduleRefreshSuggestion(input: RefreshRequest, annotations: Annotation[]) {
